        ...
        if self.settings.connect_client_id and not self.settings.secret:
            ...
            if not self.settings.connect_user_id:
                ...
                settings_content = (
                    ...
                    reverse('plugins:eventyay_paypal:oauth.disconnect', kwargs={
    ...
    def init_api(self):
        if self.settings.connect_client_id and not self.settings.secret:
            paypalrestsdk.set_config(
                ...
                openid_redirect_uri=urllib.parse.quote(build_global_uri('plugins:eventyay_paypal:oauth.return')))
        else:
            paypalrestsdk.set_config(
    ...
    def checkout_prepare(self, request, cart):
        ...
        try:
            ...
            payment = paypalrestsdk.Payment({
                ...
                "redirect_urls": {
                    "return_url": build_absolute_uri(request.event, 'plugins:eventyay_paypal:return', kwargs=kwargs),
                    "cancel_url": build_absolute_uri(request.event, 'plugins:eventyay_paypal:abort', kwargs=kwargs),
                },
                "transactions": [
                    {
                        "item_list": {
            ...
            for link in payment.links:
                if link.method == "REDIRECT" and link.rel == "approval_url":
                    if request.session.get('iframe_session', False):
                        ...
                        return (
                            build_absolute_uri(request.event, 'plugins:eventyay_paypal:redirect') + '?url=' +
